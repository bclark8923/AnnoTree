...
my $validTreeName = 'Test Tree Token Name 1';
...
ok(exists $jsonBody->{created_at},                  $testname . 'Response JSON created_at exists');
my $validTree1Token = $jsonBody->{token};
...
my $treeCreationURL = $server . $port . '/' . $validForestID . '/tree';
my $validTreeName = 'Test Tree Token Name 2';
my $validTreeDesc = 'This is a tree created by the automated Mojolicious test suite';
$tx = $uaValid->post($treeCreationURL => json => {
    name            => $validTreeName,
    description     => $validTreeDesc
...
ok('img/logo.png' eq $jsonBody->{logo},             $testname . "Response JSON logo matches");
ok(exists $jsonBody->{created_at},                  $testname . 'Response JSON created_at exists');
my $validTree2Token = $jsonBody->{token};
######### END VALID TREE CREATION TEST #########

######### START VALID TREE NAME RETRIEVAL TEST #########
# this test retrieves the names of the trees based on tokens
$testname = 'Valid tree creation 2: ';
my $treeNameURL = $server . $port . '/ios/tokens';
my $uaIOS = Mojo::UserAgent->new; # use to make the JSON POST requests
$tx = $uaIOS->get($treeNameURL => json => {
    tokens => [
        $validTree2Token, $validTree1Token, 'badtoken'
    ]
});
$jsonBody = $json->decode($tx->res->body);
print Dumper($jsonBody);
######### END VALID TREE NAME RETRIEVAL TEST #########
