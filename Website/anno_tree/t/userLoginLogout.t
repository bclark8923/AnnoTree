...
my $port = ':3000';
my $loginURL = $server . $port . '/user/login';
...
$tx = $uaValidSignUp->post($server . $port . '/user/signup' => json => {
...
$tx = $uaValidLogin->post($loginURL => json => {
...
$tx = $uaValidLogin->post($server . $port . '/user/logout');
$jsonBody = $json->decode($tx->res->body);
...
$tx = $uaInvalidLogin->post($loginURL => json => {
    signUpEmail => $validUserEmail
});
$jsonBody = $json->decode($tx->res->body);
