...
ok(exists $jsonBody->{created_at},                  $testname . 'Response JSON created_at exists');
ok($validUserID == $jsonBody->{created_by},         $testname . 'Response JSON created_by matches');
my $fullTaskID = $jsonBody->{id};
######### END FULL VALID TASK CREATION TEST #########

######### START VALID TASK DELETE TEST #########
# this test deletes an existing task
$testname = 'Valid task delete: ';
$tx = $uaValid->delete($server . $port . '/tasks/' . $minTaskID);

ok(204 == $tx->res->code,               $testname . 'Response Code is 204');
######### END VALID TASk DELETE TEST #########

######### START VALID TASK DELETE TEST #########
# this test deletes an existing task
$testname = 'Valid task delete: ';
$tx = $uaValid->delete($server . $port . '/tasks/' . $fullTaskID);
