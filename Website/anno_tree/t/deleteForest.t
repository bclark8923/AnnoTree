...
######### START VALID FOREST CREATION TEST #########
# this test creates a new forest
$testname = 'Valid forest creation 2: ';
$tx = $uaValid->post($forestURL => json => {
    name            => $validForestName,
    description     => $validForestDesc
});
...
######### START VALID FOREST DELETE TEST #########
# this test deletes an existing forest
$testname = 'Valid forest delete: ';
$tx = $uaValid->delete($server . $port . '/forest/' . $singleForestID);

ok(204 == $tx->res->code,               $testname . 'Response Code is 204');
######### END VALID FOREST DELETE TEST #########

######### START VALID FOREST DELETE TEST #########
# this test deletes an existing forest
$testname = 'Valid forest delete: ';
$tx = $uaValid->delete($server . $port . '/forest/' . $validForestID);
