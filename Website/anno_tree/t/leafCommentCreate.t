...
my $commentCreationURL = $server . $port . '/comments/leaf/' . $validLeafID;
my $validComment = 'This would be a valid comment bro on leaf ' . $validLeafID;
$tx = $uaValid->post($commentCreationURL => json => {
...
######### START INVALID LEAF COMMENT CREATION TEST #########
# this test creates a new comment
$testname = 'Valid comment creation: ';
my $invalidCommentCreationURL = $server . $port . '/comments/leaf/1';
my $validComment = 'This would be a valid comment bro on leaf ' . $validLeafID;
$tx = $uaValid->post($invalidCommentCreationURL => json => {
    comment => $validComment,
});
$jsonBody = $json->decode($tx->res->body);
...
######### END INVALID LEAF COMMENT CREATION TEST #########

######### START INVALID TEXT LEAF COMMENT CREATION TEST #########
# this test creates a new comment
$testname = 'Missing comment text comment creation: ';
$tx = $uaValid->post($commentCreationURL => json => {
    comment => '',
});
$jsonBody = $json->decode($tx->res->body);
...
ok(exists $jsonBody->{txt},     $testname . 'Response JSON error text exists');
######### END INVALID TEXT LEAF COMMENT CREATION TEST #########
